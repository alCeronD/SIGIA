...
// Implementar un autoloader para ejecutar el proceso
require_once __DIR__ . '/../../../Helpers/Const.php';
require_once BASE_URL . '/'.CR_FILE_SESSION;
require_once BASE_URL . '/'.CR_FILE_VALIDATE_PERMISOS;
require_once BASE_URL . '/'.CR_FILE_VALIDATE_FECHA;
require_once BASE_URL . '/'.CR_FILE_VALIDATE_DATA;
        ...
        if ($isOnly) {
            ...
            if (count($data) == 1) {
                ...
                if (validateFecha(fechaReservaBD:$fechaReserva, fechaReservaUser:$fechaResDB,fechaDevolBD:$fechaDevDb, fechaDevolUser:$fechaDevolucion,isFormat:true, tpPrestamo:$tpPrestamo)) {
                    fail("El elemento $elemento está reservado para la fechaSS $fechaResDB", $result);
                }else{
                    noResponse($result);
                }
