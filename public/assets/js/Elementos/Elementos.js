        ...
        mostrarConfirmacion(title, message, async (response) => {
          ...
          try {
            ...
            if (!responseInhabilitar.status) {
              initAlert(responseInhabilitar.message, 'error', toastOptions);
              return;
            }
...
const renderSelectAreas = async (action = '', inputSelect) => {
  let response = await getData('modules/elementos/controller/elementosController.php', 'GET', {
    action: action,
  });
 ...
 * @returns {boolean}
 */
const checkObject = ({ dataForm = {}, campos = {}, optionalsInput = [] } = {}) => {
